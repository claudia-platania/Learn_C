...
//IO    -->         DEVO STAMPARE TUTTI I FILM FUORI DAL WILES
...
//AGGIUNTO FINO A QUI
    ...
    while( ( c = getc(in) ) != EOF ){
        ...
        //AGGIUNTO ??  
            ...
            //SALVO IL FILM CORRENTE ED INCREMENTO IL CONTATORE

            if (film_count < numero_film) {     //eseguo / leggo tutti i film --> sino a quando il conteggio arriverà a 99
                lista_film[film_count] = current_movie;  // salvo il film in current_movie in film_list e dopo passo al successivo
                film_count++;
                //passo dentro l'array i valori di "film_count" e dopo li incremento (passo al successivo)
            }
