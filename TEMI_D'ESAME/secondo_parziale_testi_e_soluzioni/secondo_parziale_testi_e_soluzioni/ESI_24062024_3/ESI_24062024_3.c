...
int list_min(struct entry *this){ //non supporta liste vuote!
    int tmp;
    if(this->next ==NULL)//passo base
        return this->value;
    else {
        tmp = list_min(this->next);
        return (this->value < tmp) ? this->value : tmp;
    };
};

int list_sum(struct entry *this){ //non supporta liste vuote!
    if(this->next ==NULL)//passo base
        return this->value;
    else {
        list_min(this->next);
        return (this->value + list_sum(this->next));
    };
};
