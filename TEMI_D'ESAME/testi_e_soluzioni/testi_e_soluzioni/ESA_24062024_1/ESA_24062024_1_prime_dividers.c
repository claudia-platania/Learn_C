...
void print_dividers(int n, int white_list[], int size_white, int black_list[], int size_black) {
    for (int i = 0; i < size_white; i++) {
        int div = white_list[i];    //cicla ogni elemento della withe_list, assegnando ongi volta l'elemento a div

        // Soluzione ricorsiva
        if (n % div == 0 && !in(div, black_list, size_black)) {
       
            if (div > 1 && prime_r(div, div - 1)) { 
            //se div > 1 e (&&) È un numero primo   -->     chiamando la funzione prime_r
                printf("%d ", div);
            }
        }
 ...
 */
bool in(int n, int values[], int size) {
    if (size < 0) {     //se size < 0 significa che abbiamo controllato tutti gli elementi e n non è stato trovato
        return false;
    } else {
        if (n == values[size - 1]) {    //se n è uguale all'elemento corrente (in posizione size - 1) ritorna true
            return true;
        } else {
            return in(n, values, size - 1);     //altrimenti chiama se stessa per controllare l'eleemento precedente
        }
    }
}
 ...
 */
bool prime_r(int n, int i) {
    if (i == 1) {       //se i arriva a 1 significa che NESSUN numero tra 22 e n-1 ha diviso n --> n è primo
        return true;
    } else {
        if (n % i == 0) {
            return false;
